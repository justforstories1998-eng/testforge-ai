...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (subPath === 'chat-spec' && method === 'POST') {
      return await handleChatSpec(req, res);
    }
    ...
    if (subPath && !['generate', 'statistics', 'rate-limit', 'groq-status', 'models', 'chat', 'chat-spec'].includes(subPath) && method === 'GET') {
...
// Split pipeline: Qwen reads the image, GPT writes the Playwright code.
// Mirrors generateChatSpec in server/controllers/testCaseController.js.
async function handleChatSpec(req, res) {
  try {
    ...
    let analysis;
    try {
      analysis = await analyzeImageWithGroq(image, reader);
    } catch (analysisError) {
    ...
    try {
      const response = await client.chat.completions.create({
        ...
        model: writer,
        temperature: 0.3,
        max_tokens: Math.min(6000, maxOutputFor(writer)),
        ...reasoningRequestParams(writer, reasoning).apiParams,
      });
    ...
    const specCode = extractTypescript(written);
    ...
    return res.status(500).json({ success: false, error: 'AI spec generation failed. Please try again.', details: error.message });
  }
}
