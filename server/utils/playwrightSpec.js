/**
 * Playwright TypeScript spec builder (server copy).
 * KEEP IN SYNC WITH: client/src/utils/playwrightExporter.js
 *
 ...
 * locator strategies, so the spec runs against most apps as-is.
 * Each code line is preceded by a `WHY` comment explaining what it does.
 */

function normalizeRow(row) {
  return row && typeof row.toObject === 'function' ? row.toObject() : row;
}

function groupScenariosForSpec(rows) {
  ...
  safe.forEach((raw) => {
    ...
    const isHeader =
      String(row.workItemType || '').toLowerCase() === 'test case' ||
      Boolean(row.title && !row.testStep && !row.stepAction && !row.stepExpected);

    if (isHeader) {
      pushCurrent();
      current = {
        title: String(row.title || 'Untitled scenario').trim() || 'Untitled scenario',
      ...
      current.steps.push({
        action: String(row.stepAction || `Step ${row.testStep || current.steps.length + 1}`).trim(),
        expected: String(row.stepExpected || 'Verify the expected outcome.').trim(),
      });
    }
  });
  pushCurrent();
...
function buildPlaywrightSpec(testCases, options = {}) {
  const scenarios = groupScenariosForSpec(testCases);
  const timestamp = new Date().toISOString();
  const describeTitle = String(options.describeTitle || 'Test-CaseAI — Generated Suite').trim();
...
module.exports = { groupScenariosForSpec, buildPlaywrightSpec };
